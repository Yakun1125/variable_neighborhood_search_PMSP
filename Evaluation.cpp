...
double Evaluate::Calculation(int *skd)
{
    ...
    do
    {
        ...
        if (start[point] <= Data->h(skd[i]))
        {
            ...
            tardiness = Data->alpha(skd[i]) * max(start[point] - Data->d(skd[i]), 0.0);
            energy = 0;
        ...
        {
            if (Data->alpha(skd[i]) * max(start[point] + Data->b(skd[i]) + Data->a(skd[i]) - Data->d(skd[i]), 0.0) + Data->beta() * Data->b(skd[i]) * Data->q(skd[i]) <= Data->beta() * Data->gete() * (start[point] - Data->h(skd[i])) + Data->alpha(skd[i]) * max(start[point] + Data->a(skd[i]) - Data->d(skd[i]), 0.0))
            {
                start[point] = start[point] + Data->a(skd[i]) + Data->b(skd[i]);
                tardiness = Data->alpha(skd[i]) * max(start[point] - Data->d(skd[i]), 0.0);
                energy = Data->beta() * Data->q(skd[i]) * Data->b(skd[i]);
                cost[point] = cost[point] + energy + tardiness;
                i += 1;
            }
            else
            {
                energy = Data->beta() * (Data->gete() * (start[point] - Data->h(skd[i])));
                start[point] = start[point] + Data->a(skd[i]);
                tardiness = Data->alpha(skd[i]) * max(start[point] - Data->d(skd[i]), 0.0);
                cost[point] = cost[point] + energy + tardiness;
                i += 1;
